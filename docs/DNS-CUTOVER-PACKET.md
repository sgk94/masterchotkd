...
Fill these from the Resend dashboard after adding `masterchostaekwondo.com` as a sending domain.
...
Fill these from the Clerk Production domain setup.
...
```txt
NS includes ns1.vercel-dns.com and ns2.vercel-dns.com
A includes 216.198.79.1
MX includes all five Google MX records
TXT includes both Google site verification records
```
