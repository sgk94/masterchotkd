...
## Current Status
- **Phase 1 MVP: Built** — 22 pages, 11 API routes (`/api/contact` + 8 PDF downloads + 2 admin invitation routes), CI/CD, deployed to Vercel
...
- **Animation:** CSS only (scroll reveals via `<Reveal>`, marquee, border-spin, hero entrance, `offer-glow` breathing shadow on special-offer CTA)
...
- `/special-offer` — Trial ($49 / 2 weeks); asymmetric 12-col hero (editorial headline left, oversized $49 display right), numbered benefit data strip (01–04), kinetic marquee strip below hero, CTA with breathing `offer-glow` shadow + directional arrow
...
- `/members/curriculum/color-belt` — Beginner/Intermediate/Advanced cycle breakdown (card-based); Poomsae Videos section with `<PoomsaeCard>` + `<YouTubeFacade>` (click-to-load iframe, youtube-nocookie.com); Korean form names (태극 일장, etc.) + cycle-ring progress indicator + form index (01/08); 2 videos wired (Taegeuk 1 + 2), 7 placeholders
...
- `POST /api/admin/invitations` — admin-only, creates a Clerk invitation and emails the recipient. Body read as text + `Buffer.byteLength` cap (not trusted `Content-Length`). Rate-limit key includes `userId:ip`. Status codes: 201 success; 400 invalid email or JSON; 401 signed out; 403 not admin or bad origin; 413 body > 2KB; 429 rate-limited; 409 duplicate email; 502 Clerk error. Body: `{ email: string }`. Uses `clerkErrorToResponse()` to map Clerk error codes (duplicate_record → 409, resource_not_found → 404).
- `DELETE /api/admin/invitations/[id]` — admin-only, revokes the invitation. validateOrigin (CSRF). 200 ok; 400 malformed id; 401/403 as above; 404 not found; 502 Clerk error.
...
- `src/lib/` — `db`, `server-env` (lazy `getServerEnv()`), `client-env`, `fonts`, `metadata`, `email` (5s Resend timeout), `rate-limit` (fail-open on backend errors), `sanitize` (+ `escapeHtml`), `static-data`, `api-security` (`validateOrigin` + `getClientIp`), `clerk-admin` (`requireAdmin` + `isAdminUser` + `clerkErrorToResponse`), `members-home-content`, `current-cycle`, `current-cycle-materials`, `location`, `nav` (single nav source).
...
- Special-offer page has no `<PageContainer>` — custom layout lets the kinetic marquee strip sit full-bleed below the hero section.
...
## Tests
Vitest (`pnpm vitest run`) + Playwright E2E (`tests/e2e/*`, needs running app). 257 tests / 56 files. Coverage gate: 75% branches (CI enforced). Coverage spans contact + invitation schemas, `/api/contact` + `/api/admin/invitations` route branches, `clerk-admin` guard, rate-limit fail-open, `current-cycle` boundaries, component rendering (navbar, hero, programs-grid, schedule-grid, red-black + black-belt-club pages, members-tab-bar, resource-card, admin invitations page, invite-form, YouTubeFacade), protected PDF route, `next.config` flags, `globals.css` grain mobile gate, image size budget.
